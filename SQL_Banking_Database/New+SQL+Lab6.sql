...
(102,'Priya Singh', 'HR', 'HR Manager', 75000, '2021-05-20', 'Mumbai'),
...
select emp_name, substr(emp_name,2,4),substring(emp_name,-1,4) from company;
...
-- 1) Greatest(): returns the largest value
	select
    department,
    salary,
    ...
    select greatest(20000,300000,2325,45532,899998,999888) as greatest_value;
    
-- 2) Least(): returns lowest value
	select least(12,32,11,23,22,54); 
...
-- Distinct() : returns unique value of columns
   ...
   -- Salary increased by 25% --
    ...
    select salary, salary*(1-0.1), salary*0.9 as 'salary DECREASED BY 10%'
    from company;
...
## Aggregate function in sql : perform calculation on multiple rows
    ...
    select department, avg(salary) as 'total salary'
    from company
    group by department;
